 ...
 * 
 * 
 * @rule : warn 檢核保單是否存在尚未歸檔的受理案件，受理編號尚未歸檔
 * 
 * 
 */
public class ScPtValidateServiceImpl implements ScPtValidateService {
    ...
    public ScPtValidateResult validateNonacceptanceTransfer(ScPtValidateContext context, 
        ...
        /** 此保單需要的資格證種類 */
        final Set<Integer> types = this.getQualificationTypes(vo.getPolicyId(), 
                        policy.getApplyDate());
        ...
        if (vo.getNewServiceRegisterId() != null) {
            if (!vo.getNewServiceRegisterId().equals(vo.getOriAgentId())) {
                ...
                result.addIgnoreNull(ScPtMsgVO.decorator(NEW_SERVICE_MSG_CODE, 
                                this.validateAgentNotGrpFake(serviceAgent)));
                result.addIgnoreNull(this.validateAgent(context, NEW_SERVICE_MSG_CODE, serviceAgent));
                result.addIgnoreNull(ScPtMsgVO.decorator(NEW_SERVICE_MSG_CODE, 
                                validateDirectorCfmDateAndProbationDate(vo.getNewStdDateDate(), oriAgent, serviceAgent)));
                final Set<Integer> agentTestType = this.getAgentTestType(context, serviceAgent.getAgentId());
                result.addIgnoreNull(this.validateAgentTest(serviceAgent, NEW_SERVICE_MSG_CODE, types, agentTestType));
        ...
        if (vo.getNewCommRegisterId() != null) {
            if (!vo.getNewCommRegisterId().equals(vo.getOriAgentId())) {
                ...
                result.addIgnoreNull(ScPtMsgVO.decorator(NEW_COMM_MSG_CODE, 
                                this.validateAgentNotGrpFake(commAgent)));
                result.addIgnoreNull(this.validateAgent(context, NEW_COMM_MSG_CODE, commAgent));
                result.addIgnoreNull(ScPtMsgVO.decorator(NEW_COMM_MSG_CODE, 
                                validateDirectorCfmDateAndProbationDate(vo.getNewStdDateDate(), oriAgent, commAgent)));
                
                final Set<Integer> agentTestType = this.getAgentTestType(context, commAgent.getAgentId());
                result.addIgnoreNull(this.validateAgentTest(commAgent, NEW_COMM_MSG_CODE, types, agentTestType));
        ...
        result.addIgnoreNull(this.warnCaseExist(vo.getPolicyId()));
    ...
    public ScPtValidateResult validateAllocate(ScPtValidateContext context, ScPolicyTransferVO vo) {
        ...
        if (vo.getNewServiceRegisterId() != null) {
            if (!vo.getNewServiceRegisterId().equals(caseVO.getAgentId())) {
                ...
                result.addIgnoreNull(ScPtMsgVO.decorator(NEW_SERVICE_MSG_CODE, 
                                validateDirectorCfmDateAndProbationDate(vo.getNewStdDateDate(), oldAgent, serviceAgent)));
        ...
        if (vo.getNewCommRegisterId() != null) {
            if (!vo.getNewCommRegisterId().equals(caseVO.getAgentId())) {
                ...
                result.addIgnoreNull(ScPtMsgVO.decorator(NEW_COMM_MSG_CODE, 
                                validateDirectorCfmDateAndProbationDate(vo.getNewStdDateDate(), oldAgent, commAgent)));
        ...
        result.addIgnoreNull(this.validateDirectorCfmDateLargerThanSystemDate(vo.getNewStdDateDate()));
        result.addIgnoreNull(this.warnCaseExist(vo.getPolicyId(), caseVO.getCaseId()));
    ...
    public ScPtValidateResult validateBatchUploadTransfer(final ScPtValidateContext context, final SingleCommonPtFileVO row) {
        ...
        if (!result.hasError()) {
            ...
            if (!ScPtFileTransferType.MODIFY.equals(row.getPtPolicyType())) {
                ...
                /** 此保單需要的資格證種類 */
                final Set<Integer> types = this.getQualificationTypes(row.getPolicyId(), 
                                policy.getApplyDate());
                ...
                if (StringUtils.isNotEmpty(row.getNewServiceRegisterCode1())) {
                    ...
                    if (newAgent == null) {
                        ...
                        if (!newAgent.getAgentId().equals(row.getFromAgentId())) {
                            ...
                            final Set<Integer> agentTestType = this.getAgentTestType(context, newAgent.getAgentId());
                            result.addIgnoreNull(this.validateAgentTest(newAgent, NEW_SERVICE_MSG_CODE, types, agentTestType));
                ...
                if (StringUtils.isNotEmpty(row.getNewCommRegisterCode())) {
                    ...
                    if (newAgent == null) {
                        ...
                        if (!newAgent.getAgentId().equals(row.getFromAgentId())) {
                            ...
                            final Set<Integer> agentTestType = this.getAgentTestType(context, newAgent.getAgentId());
                            result.addIgnoreNull(this.validateAgentTest(newAgent, NEW_COMM_MSG_CODE, types, agentTestType));
                ...
                result.addIgnoreNull(this.warnCaseExist(policy.getPolicyId()));
...
//                            vo.getNewServiceAgent())));
        ...
        if (vo.isTransferToExternal()) {
            ...
            result.addIgnoreNull(this.validateAgentTest(context, vo.getNewBrbdServiceAgent(), SERVICE_MSG_CODE, types));
...
//            result.addIgnoreNull(this.warnAcceptanceReason(vo.getChgVO().getAcptReason()));
            ...
            result.addIgnoreNull(this.validateAgentTest(context, vo.getNewServiceAgent(), SERVICE_MSG_CODE, types));
            
            
            result.addIgnoreNull(this.validateAgent(vo.getNewCommAgent(), COMMISSION_MSG_CODE));
            result.addIgnoreNull(this.validateAgentTest(context, vo.getNewCommAgent(), COMMISSION_MSG_CODE, types));
        }
    ...
    public ScPtValidateResult validateBRBDBatchUploadTransfer(final ScPtValidateContext context, BRBDCommonPtFileVO row) {
        ...
        //服務與領佣須相同
        if (!isForceTransfer && result.addIgnoreNull(this.validatePolicyServiceCommEqual(policy.getPolicyId()))) {
            return result;
        }
        ...
        if (StringUtils.isNotEmpty(row.getAgentCode())) {
            ...
            if (agentChl != null) {
                ...
                /** 此保單需要的資格證種類 */
                final Set<Integer> types = this.getQualificationTypes(policy.getPolicyId(), 
                                policy.getApplyDate());
                
                final Set<Integer> agentTypeSet = this.getAgentTestType(context, agentChl.getAgentId());
                
                result.addIgnoreNull(validateAgentTest(agentChl, NEW_SERVICE_MSG_CODE, types, agentTypeSet));
     ...
     * @return
     */
    private ScPtMsgVO validateCommonChoiceX(long policyId, AgentChlVO oriAgent, CommChoice newCommonChoice) {
        ScPtMsgVO msg = null;
        
        if (newCommonChoice != null && !CommChoice.X.equals(newCommonChoice) && oriAgent != null) {
...
//            }
    ...
    private ScPtValidateResult validateAgentTest(ScPtValidateContext context,
                    ...
                    final String[] prefix, Set<Integer> types) {
        final ScPtValidateResult result = new ScPtValidateResult();
        int i = 0;
        for  (final ScPtNewRoleVO vo : newAgentRoles) {
            final AgentChlVO agentVO = vo.getAgentChlVO(agentScService);
            
            final Set<Integer> agentTypeSet = this.getAgentTestType(context, agentVO.getAgentId());
            
            //normat agent
            result.addIgnoreNull(this.validateAgentTest(agentVO, prefix[i], types, agentTypeSet, false));
            i++;
        }
        return result;
    }
     ...
     * @return
     */
    private ScPtValidateResult validateAgentTest(final AgentChlVO agentVO, 
                    final String prefix, Set<Integer> types, final Set<Integer> agentTypeSet, boolean fakeParameter) {
        final ScPtValidateResult result = new ScPtValidateResult();
     ...
     * <RULE_SET>
     * 	  	<RULE level="error">此保單服務與領佣不同不可移轉</RULE>
     * </RULE_SET>
     * <p>Created By : TGL155</p>
     * <p>Create Time : Dec 6, 2016</p>
     * @param policyId
     ...
     * @return
     */
    private ScPtMsgVO validatePolicyServiceCommEqual(long policyId) {
        ScPtMsgVO msg = null;
        
        if (!policyProducerRoleService.isPolicyServiceCommSame(policyId)) {
            msg = ScPtMsgVO.error("MSG_1262394"); //此保單服務與領佣不同不可移轉
        }
        return msg;
    }
...
//    }
    ...
    private ScPtMsgVO validateShareCount(long policyId) {
        ...
        if (policyProducerRoleService.checkThreeRoleShareCase(policyId)) {
            msg = ScPtMsgVO.error("MSG_1261355");
        }
...
//    }
    ...
    private ScPtMsgVO warnExternalServiceAgentChannelOrg(ChannelOrgVO serviceRoleChannelOrg,
        ...
        if (!ObjectUtils.equals(serviceRoleChannelOrg.getParentId(), agentRoleChannelOrg.getParentId())) {
            msg = ScPtMsgVO.warn("MSG_1256128");
//            		.append(" : " + agentRoleChannelOrg.getChannelCode()); //須相同經代公司
        }
     ...
     * @return
     */
    private ScPtValidateResult warnCaseExist(long policyId, Long caseId) {
        ScPtValidateResult result = new ScPtValidateResult();
                        
        List<Map<String, Object>> caseInfo = scPtQueryService.getUnArchiveCasePolicy(policyId);
     ...
     * @return
     */
    private ScPtValidateResult warnCaseExist(long policyId) {
        return this.warnCaseExist(policyId, null);
    }
...
//    }
     ...
     * @return
     */
    private Set<Integer> getQualificationTypes(long policyId, Date applyDate) {
        Set<Integer> types = new HashSet<Integer>();
        Set<Integer> productIds = contractMasterService.getDistinctProductId(policyId);

        for (Integer productId : productIds) {
            com.ebao.ls.pa.pub.ref.vo.prd.ProductVO product = productService.getProduct(productId.longValue(), applyDate);
            // Get product's qualification type.
            List<Integer> qTypes = product.getQualificationTypes();
            for (Integer type : qTypes) {
                types.add(type);
            }
        }
        return types;
    ...
    private Set<Integer> getAgentTestType(ScPtValidateContext context, long agentId) {
        ...
        if (context.getAgentTest(agentId) == null) {
            final Set<Integer> agentTypeSet = individualAgentService.getAgentValidateTestInfoTypeSet(agentId);
            context.cacheAgentTest(agentId, agentTypeSet);
        }
